/**
 * Gera fotos de exemplo para todos os estilos do Ensaio Fotográfico
 * usando uma modelo de referência, e salva no Supabase.
 *
 * Usage: node scripts/generate-ensaio-examples.mjs /path/to/model.png
 */
...
const STYLES = [
  ...
  { id: "urban-reflection", label: "Urban Reflection", prompt: `Dramatic ultra-realistic close-up in black and white with high-contrast cinematic lighting from the side, highlighting the contours of the face and neck. Subject is the person from the attached photo — do not alter face, features, or identity in any way. Background fades to deep black. Sharp focus on eyes and facial structure. Intense, artistic, editorial quality. Studio photography.` },
  { id: "monochrome-editorial", label: "Editorial Minimalista", prompt: `Black-and-white editorial portrait of the person in the attached photo — keep their face 100% identical. The person sits on a minimalist wooden block inside a modern loft apartment. Wearing a fitted black turtleneck. Side rim lighting creating precise shadows. Clean, geometric composition. Magazine-quality monochrome photography, ultra-sharp.` },
  { id: "perfil-pb", label: "Perfil Cinematografico", prompt: `Cinematic black-and-white portrait of the person in the attached photo — exact face, unchanged. The person is shown in side profile, leaning against a smooth wall with a confident, composed posture. The lighting creates a rim effect along the profile. Clean, minimal composition with deep blacks and bright highlights. Editorial, dramatic, artistic photography.` },
  ...
  { id: "mulher-negocios", label: "Mulher de Negocios", prompt: `Recreate this scene using the face and body from the attached photo as reference — same face, same identity. A confident businesswoman sitting on a luxury chair in a modern upscale office. She wears a tailored blazer over a silk blouse. Professional yet elegant. Dramatic side lighting, editorial quality. Ultra-realistic 4K photography.` },
  { id: "mulher-elegante", label: "Elegante e Sofisticada", prompt: `Recreate an elegant scene using the face and appearance from the attached photo — maintain exact identity. A sophisticated woman in a high-fashion setting: standing in a luxurious interior with marble walls, wearing an elegant outfit. Soft dramatic lighting highlighting the face. Fashion editorial quality, ultra-realistic.` },
  { id: "estudio-feminino", label: "Studio Portrait", prompt: `A high-impact black and white studio portrait of the person from the attached photo — face preserved exactly. The female figure is photographed in an intimate close-up with a pensive, introspective expression. Single dramatic studio light from the side creates strong shadows. Clean studio backdrop. Fine art photography quality, ultra-sharp.` },
];

async function b64ToBuffer(b64, mimeType) {
...
async function generate(photoDataUrl, style) {
...
async function main() {
  const modelPath = process.argv[2] ?? "/Users/omarcoswillian/Downloads/vitoria.png";
  const ext = modelPath.split(".").pop()?.toLowerCase() ?? "png";
  ...
  console.log(`Modelo: ${modelPath} (${(rawBytes.length / 1024).toFixed(0)} KB)`);
  console.log(`Gerando ${STYLES.length} estilos...\n`);
  ...
  for (let idx = 0; idx < STYLES.length; idx++) {
    ...
    process.stdout.write(`[${idx + 1}/${STYLES.length}] ${style.label}... `);
    try {
      const { b64, mimeType } = await generate(photoDataUrl, style);
      const { buffer } = await b64ToBuffer(b64, mimeType);
  ...
  console.log(`\nConcluido: ${done} gerados, ${failed} falharam.`);
}
