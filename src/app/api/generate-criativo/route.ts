...
const FORMAT_CONFIG: Record<CriativoFormat, { size: string; platform: string; instructions: string }> = {
  "youtube-thumbnail": {
    size: "1536x1024",
    platform: "YouTube",
...
function buildPrompt(
  ...
  const faseCtx = FASE_CONTEXT[fase] ?? "";
  const estiloCtx = ESTILO_CONTEXT[estilo] ?? "";
  ...
  try {
    ...
    if (!key) {
      return NextResponse.json(
        { error: "Chave OpenAI não configurada. Adicione em Configurações > BYOK ou defina OPENAI_API_KEY." },
        { status: 400 }
      );
    }
    ...
    if (referenceBase64) {
      // Edit mode: use reference image with gpt-image-2 edit
    ...
    if (msg.includes("content_policy") || msg.includes("safety")) return NextResponse.json({ error: "Prompt bloqueado pela OpenAI. Reformule." }, { status: 400 });
